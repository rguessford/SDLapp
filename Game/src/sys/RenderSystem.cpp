...
void RenderSystem::update(double dt) {
	...
	mRegistry.view<position, camera>().each([&](auto& camera, auto& campos) {
		...
		mRegistry.view<tileMapRenderer, drawable>().each([&](auto& mapRenderer, const auto& drawable) {
			for (int i = 0; i < 100; i++) {
				for (int j = 0; j < 100; j++) {
					GPU_Rect renderquad = { i * 32 ,j * 32,64,64 };
					twoDToIso(&renderquad);
					renderquad.x += campos.xOffset;
					renderquad.y += campos.yOffset;
					GPU_BlitRect(drawable.textureDetails->texture, &drawable.textureDetails->frameMapping->at(mapRenderer.tileMap[j][i]), mRenderer, &renderquad);
				}
			}
			});
		//render actors
		...
		mRegistry.view<drawable, position>().each([&](auto& drawable, const auto& position) {
			...
			GPU_Rect renderquad = { (int)position.x , (int)position.y,128,128 };
			twoDToIso(&renderquad);
			renderquad.x -= drawable.textureDetails->foot.x;
			renderquad.y -= drawable.textureDetails->foot.y;
			renderquad.x += campos.xOffset;
			renderquad.y += campos.yOffset;
			GPU_BlitRect(drawable.textureDetails->texture, &drawable.textureDetails->frameMapping->at(drawable.currentFrame), mRenderer, &renderquad);
		});
	});
